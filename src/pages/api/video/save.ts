...
function renameUploadedFile(
  ...
  const uploadBasePath = `./public/uploads/${contentsId}/`;
...
export default async function handler(
  ...
  // contentsId를 먼저 추출
  let contentsId = "anonymous";
  if (req.url && req.url.includes("contents_id=")) {
    const match = req.url.match(/contents_id=([^&]+)/);
    if (match && match[1]) contentsId = match[1];
  }
  // formidable 인스턴스 생성 시 contentsId 사용
  const form = formidable({
    uploadDir: `./public/uploads/${contentsId}`,
    keepExtensions: true,
    multiples: true,
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    const contentsId = String(fields.contents_id || "anonymous");
    ...
    try {
      fileArray.forEach((file) => {
        if (file) {
          const newName = renameUploadedFile(file, userId, contentsId);
